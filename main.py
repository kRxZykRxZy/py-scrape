...
def db():
 ...
 # Upgrade databases created before job_id existed.
...
def scrape_job(jid,p,a):
 with DB_LOCK:c=db();c.execute('update scrape_jobs set status="running",started_at=? where id=?',(time.time(),jid));job_log(c,jid,f'Started {p}, target {a}');c.close()
 try:
  ...
  with DB_LOCK:c=db();c.execute('update scrape_jobs set status="complete",found=?,saved=?,finished_at=? where id=?',(found,saved,time.time(),jid));job_log(c,jid,f'Finished: {saved} leads saved');c.close()
 except Exception as e:
  with DB_LOCK:c=db();c.execute('update scrape_jobs set status="error",error=?,finished_at=? where id=?',(str(e),time.time(),jid));job_log(c,jid,'ERROR: '+str(e));c.close()
...
def status():
 with DB_LOCK:
  c=db();total=c.execute('select count(*) from leads').fetchone()[0];jobs=c.execute('select * from scrape_jobs order by coalesce(started_at,0) desc').fetchall();c.close()
 return jsonify(total=total,jobs=[dict(x) for x in jobs],running=any(x['status'] in ('queued','running') for x in jobs))
@app.get('/api/jobs/<jid>')
def job(jid):
 with DB_LOCK:c=db();r=c.execute('select * from scrape_jobs where id=?',(jid,)).fetchone();c.close()
 return (jsonify(dict(r)) if r else (jsonify(error='Job not found'),404))
@app.get('/api/leads')
def leads():
 q=request.args.get('search','').strip();s=request.args.get('status','').strip();jid=request.args.get('job_id','').strip()
 with DB_LOCK:
  c=db();sql='select * from leads';params=[];w=[]
  if jid:w.append('job_id=?');params.append(jid)
  if q:w.append('(name like ? or phone like ? or email like ? or address like ? or category like ?)');params+=['%'+q+'%']*5
  if s in ALLOWED_STATUS:w.append('status=?');params.append(s)
