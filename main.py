...
def db():
 c=sqlite3.connect(DB_PATH);c.row_factory=sqlite3.Row;c.execute('''create table if not exists leads(id integer primary key autoincrement,name text not null,category text,phone text,email text,address text,website text,status text default 'new',created_at text default current_timestamp)''');c.commit();return c
def log(x): state['logs'].append(time.strftime('%H:%M:%S ')+x);state['logs']=state['logs'][-200:]
def scrape(postcode,amount):
 state.update(running=True,state='scraping',found=0,saved=0,logs=[],started_at=time.time());log(f'Started {postcode}, target {amount}')
 try:
  from scraper.maps import search_google_maps
  rows=search_google_maps(postcode,amount);c=db();seen=set()
  for r in rows:
   state['found']+=1;name=(r.get('name') or '').strip();key=name.lower()
   if not name or key in seen or r.get('website'):continue
   seen.add(key);c.execute('insert into leads(name,category,phone,email,address,website) values(?,?,?,?,?,?)',(name,r.get('category',''),r.get('phone',''),r.get('email',''),r.get('address',''),r.get('website','')));state['saved']+=1
...
@app.get('/api/leads')
def leads():
 q=request.args.get('search','').strip();status=request.args.get('status','').strip();c=db();sql='select * from leads';params=[];where=[]
 if q:where.append('(name like ? or phone like ? or email like ? or address like ? or category like ?)');params += ['%'+q+'%']*5
 if status in ALLOWED_STATUS:where.append('status=?');params.append(status)
 if where:sql+=' where '+' and '.join(where)
 sql+=' order by id desc';rows=c.execute(sql,params).fetchall();c.close();return jsonify(leads=[dict(x) for x in rows])
@app.post('/api/scrape')
def start():
 d=request.json or {};p=str(d.get('postcode','')).upper().strip();a=int(d.get('amount',0) or 0)
 if not p or not a or a<1 or a>MAX_LEADS:return jsonify(error=f'Amount must be 1-{MAX_LEADS}'),400
