...
def _area(outcode):
 ...
 if not prefix or prefix.group(0) not in LONDON_PREFIXES: raise ValueError("Only London postcode districts are supported")
 result=_request(POSTCODES_URL.format(outcode)).get("result")
 if not result or result.get("admin_district") not in LONDON_BOROUGHS: raise ValueError("That postcode district is outside the supported London area")
 ...
 return b,result.get("admin_district","")
